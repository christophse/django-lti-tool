...
from lti_tool.exceptions import (
    LTIKeyRetrieveError,
    LTINoLineItem,
    LTIRequestError,
    LTIResourceError,
...
class Platform(Updatable):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["issuer", "deployment_id"], name="unique_lti_platform"
            )
        ]
    ...
    def keyset(self):
        try:
            ...
            raise LTIKeyRetrieveError("Could not retrieve platform keyset.") from e

        return jwk.JWKSet.from_json(resp.text)

    @staticmethod
    def get_fields(claims):
        claim = claims.get("https://purl.imsglobal.org/spec/lti/claim/tool_platform")
...
class Context(Updatable):
    ...
    platform = models.ForeignKey(Platform, editable=False, on_delete=models.CASCADE)
    label = models.CharField(max_length=255, editable=False, default="", blank=True)
    title = models.CharField(max_length=255, editable=False, default="", blank=True)
    context_type = models.JSONField(editable=False, default=list)
    scope = models.JSONField(editable=False, default=list)
    _lineitems = models.CharField(
        max_length=255, editable=False, default="", blank=True
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["context_id", "platform"], name="unique_lti_context"
            )
        ]
...
class Resource(Updatable):
    ...
    context = models.ForeignKey(
        Context, editable=False, null=True, on_delete=models.CASCADE
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["resource_id", "platform"], name="unique_lti_resource"
            )
        ]
    ...
    def lineitem(self):
        ...
        if not self.lineitem_id:
            raise LTINoLineItem(
                "No line item available. Make sure assigment "
    ...
    def __str__(self):
        ...
        return f"{self.title}, Context: {context}, " f" Platform: {self.platform}"

    @staticmethod
    def get_fields(claims):
        msg_type = claims["https://purl.imsglobal.org/spec/lti/claim/message_type"]

        if not (
            msg_type == "LtiResourceLinkRequest"
...
class LTIUser(Updatable):
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="lti")
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["identifier", "platform"], name="unique_lti_user"
            )
        ]
    ...
    def get_base_fields(claims):
        ...
        mail_ext = claims.get("https://purl.imsglobal.org/spec/lti/claim/ext", {}).get(
            "email"
        )
...
class Roles(Updatable):
    ...
    @staticmethod
    def get_fields(claims):
        return {"roles": claims["https://purl.imsglobal.org/spec/lti/claim/roles"]}

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["lti_user", "context"], name="unique_lti_role"
            )
        ]
