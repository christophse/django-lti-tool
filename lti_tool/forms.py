...
class KeyForm(forms.ModelForm):
    priv_key = forms.CharField(
        label='Private key',
        help_text=_('Private key in <strong>PKCS#8 format</strong>. A new key '
    ...
    pub_key = forms.CharField(
        label='Public key',
        help_text=_('<strong>Read only.</strong> Will be generated after '
        ...
        if cleaned_data['priv_key']:
            try:
                ...
                raise ValidationError({
                    'priv_key': _('Not a valid key. Please provide a key in '
                                  'PKCS#8 format.')
                })
