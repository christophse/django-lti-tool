...
class HTTPClient:
    ...
    def _get_access_token(self, platform, scope):
        header = {
            'alg': 'RS256',
            'type': 'JWT',
            'kid': platform.key.kid
        }
    ...
    def _get_auth_header(self, context):
        ...
        if not access_token or scope != context.scope:
            data = self._get_access_token(context.platform, context.scope)
